...
void BattleLevel::BeginPlay()
{
	...
	mainController->SetLocalPosition(Vector2D(400.f, 300.f));

	Sprite* background0 = GameplayStatics::CreateObject<Sprite>();
	background0->LoadSprite("bg0");
	background0->SetLocalPosition(Vector2D(475, 300));
	...
	GameplayStatics::CreateObject<SunFlower>(Vector2D(500, 500));
}
